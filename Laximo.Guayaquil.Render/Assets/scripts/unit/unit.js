...
function SubscribeDblClick(selector) {
	jQuery(selector).dblclick(function() {
		...
		var items = jQuery('tr[name=' + el_name + ']');
		...
		if (items.length == 1) {
			...
			$.colorbox({
				'html': function() {
					var items = jQuery('tr[name=' + el_name + '] td[name=c_name]');
					var name = jQuery(items[0]).text();

					var html = '<h2><span>' + name + '</span></h2>' +
                        '<table>';

					var oems = jQuery('tr[name=' + el_name + '] td[name=c_oem]');
					var notes = jQuery('tr[name=' + el_name + '] td[name=c_note]');
					var urls = jQuery('tr[name=' + el_name + '] td[name=c_oem] a.follow');
					...
					for (idx = 0; idx < count; idx++) {
						...
						url += (url.indexOf('?') >= 0 ? '&' : '?') + 'format=raw';
						html += '<tr><td><a href="#" onclick="$.colorbox({\'href\': \'' + url + '\',\'opacity\': 0.3, \'innerWidth\' : \'1000px\',\'maxHeight\' : \'98%\'}); return false;">' + jQuery(oems[idx]).text() + '</a></td><td>' + jQuery(notes[idx]).text() + '</td></tr>';
					}
